...
const CartInfo = () => {
    ...
    // ✅ FIXED: Send only the unit price fields, not the accumulated item
    ...
    const handleAdd = (item) => {
        dispatch(addItems({
            ...
            pricePerQuantity: item.pricePerQuantity,  // ✅ unit price only
            quantity: 1,                               // ✅ always add 1 unit
            section: item.section,
            orderNo: item.orderNo,
            // ✅ DO NOT spread `item` — item.price is the accumulated total
            //    and would overwrite pricePerQuantity via fallback logic
        }));
    }
    ...
    return (
        <div className='flex flex-col' style={{ maxHeight: 'calc(100vh - 600px)', minHeight: '150px' }}>
                    ...
                    <div className='space-y-2 lg:space-y-1.5 xl:space-y-2 pb-2'>
                        {cartData.map((item, index) => (
                            ...
                            >
                                ...
                                {/* ✅ Show unit price breakdown for clarity */}
                                <div className='flex items-center justify-between mt-1'>
                                    <p className='text-[#666] text-xs lg:text-[10px]'>
                                        BHD {(item.pricePerQuantity || 0).toFixed(3)} × {item.quantity}
                                    </p>
                                </div>
                                ...
                                <div className='flex items-center justify-between mt-2 lg:mt-1.5 xl:mt-2'>
                                    ...
                                    {/* ✅ Always show recomputed total, never trust item.price directly */}
                                    <p className='text-[#f5f5f5] text-sm lg:text-[11px] xl:text-xs 2xl:text-sm font-bold'>
                                        BHD {((item.pricePerQuantity || 0) * (item.quantity || 0)).toFixed(3)}
                                    </p>
                                </div>
                            </div>
                        ))}
            ...
            <style jsx>{`
                ...
                div::-webkit-scrollbar-track { background: #1f1f1f; border-radius: 10px; }
                div::-webkit-scrollbar-thumb { background: #3a3a3a; border-radius: 10px; }
