...
// import socket from "../socket";
// import { useNavigate } from 'react-router-dom';
// import { ChevronDown, Eye, CheckCircle, WifiOff, Wifi, Printer } from 'lucide-react';
// import { getCachedOrders, fetchAndCacheRecentOrders, startAutoRefresh, stopAutoRefresh } from "../utils/getOrdersOffline";
// import { getOfflineOrders } from "../utils/offlineOrders";
// import { printSalesReport } from "../https/printBridge";
...
//     // extra hook that always asks for In-Progress count
...
//         isOnline
//     );
//     console.log('📊 bulkStats', bulkStats);  
...
//         totalOrders
//     } = useInfiniteOrders(filters, isOnline);
//     const { stats: orderStats, isLoading: isLoadingStats, refetch: refetchStats } = useOrderStats(filters, isOnline);
//     const {
//         totals: paymentTotals,
//         counts: paymentCounts,
//         isLoading: isLoadingPaymentTotals,
//         refetch: refetchPaymentTotals
//     } = usePaymentTotals(filters, isOnline);
...
//             stopAutoRefresh();
//         };
//     }, [refetchOrders, refetchStats, refetchPaymentTotals]);
...
//             const relevantActions = ['new_order', 'items_ready', 'status_changed', 'order_modified', 'order_deleted'];
//             if (relevantActions.includes(data.action)) {
//                 refetchOrders();
//                 refetchStats();
//                 refetchPaymentTotals();
...
//                     'order_deleted': 'Order deleted'
//                 };
//                 enqueueSnackbar(actionMessages[data.action] || 'Order updated', {
//                     variant: data.action === 'new_order' ? 'success' : 'info',
//                     autoHideDuration: 3000
//                 });
//             }
...
//     }, [isOnline, refetchOrders, refetchStats, refetchPaymentTotals]);
...
//             return [...combined, ...pendingWithFlag];
//         }
//     }, [isOnline, infiniteOrders, offlineOrders, pendingOrders]);
...
//     const completableOrders = useMemo(() => {
//         return sortedFilteredOrders.filter(
//             order => order.orderStatus !== "Completed" && order.orderStatus !== "Cancelled" && !order._isPending
//         );
//     }, [sortedFilteredOrders]);

//     // Handle sales report printing
...
//         setIsPrintingReport(true);
//         try {
//             // Use the optimized analytics data if available, otherwise calculate manually
//             const reportData = todayData || calculateSalesReportManually();
//             await printSalesReport(reportData);
//             enqueueSnackbar("Sales report printed successfully!", { variant: "success" });
//         } catch (error) {
//             console.error("Error printing sales report:", error);
...
//     const calculateSalesReportManually = useCallback(() => {
//         const today = new Date().toDateString();
//         const todayOrders = sortedFilteredOrders.filter(order => {
//             const orderDate = new Date(order.createdAt).toDateString();
//             return orderDate === today && order.orderStatus === "Completed";
//         });

//         // Calculate totals using the same logic as before
//         const orderTypes = { dinein: 0, dineinCount: 0, takeaway: 0, takeawayCount: 0, delivery: 0, deliveryCount: 0, total: 0 };
//         const paymentMethods = { cash: 0, cashCount: 0, online: 0, onlineCount: 0, benefit: 0, benefitCount: 0, total: 0 };
//         let totalTax = 0;
//         let totalDiscount = 0;
//         const itemsMap = new Map();
//         const hourlyBreakdown = {};
...
//         // Format and return the report data
//         orderTypes.total = orderTypes.dinein + orderTypes.takeaway + orderTypes.delivery;
//         paymentMethods.total = paymentMethods.cash + paymentMethods.online + paymentMethods.benefit;
//         const totalSales = orderTypes.total;
//         const totalOrders = todayOrders.length;
...
//                 takeawayCount: orderTypes.takeawayCount,
//                 delivery: orderTypes.delivery.toFixed(3),
//                 deliveryCount: orderTypes.count,
//                 total: orderTypes.total.toFixed(3)
//             },
//             paymentMethods: {
//                 cash: paymentMethods.cash.toFixed(3),
...
//                 total: paymentMethods.total.toFixed(3)
//             },
//             taxSummary: { total: totalTax.toFixed(3) },
//             discountSummary: { total: totalDiscount.toFixed(3) },
//             topItems: Array.from(itemsMap.values()).sort((a, b) => b.revenue - a.revenue).slice(0, 5),
//             hourlyBreakdown: Object.keys(hourlyBreakdown).sort().reduce((acc, hour) => {
//                 acc[hour] = {
//                     sales: hourlyBreakdown[hour].sales.toFixed(3),
//                     count: hourlyBreakdown[hour].count
//                 };
...
//             totalSales: totalSales.toFixed(3)
//         };
//     }, [sortedFilteredOrders, todayData]);
...
//     const handleBulkCompleteClick = useCallback(() => {
//         if (!isOnline) {
//             enqueueSnackbar("You must be online to complete orders!", { variant: "error" });
//             return;
//         }
...
//         }
//         setShowBulkCompleteModal(true);
//     }, [isOnline, completableOrders.length]);

//     // Loading state
//     if ((isLoadingOrders && isOnline) || isLoadingOffline) {
//         return (
//             <section className="bg-[#1f1f1f] h-[calc(100vh-5rem)] flex items-center justify-center">
//                 <div className="text-center">
//                     <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#02ca3a] mx-auto mb-4"></div>
...
//                         Orders History
//                     </h1>
//                     <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold ${isOnline ? 'bg-green-500/20 text-green-400' : 'bg-orange-500/20 text-orange-400'
//                         }`}>
//                         {isOnline ? <Wifi className="w-4 h-4" /> : <WifiOff className="w-4 h-4" />}
//                         {isOnline ? 'Online' : 'Offline'}
//                     </div>
...
//                             </p>
//                             <p className="text-xs text-orange-300">
//                                 Showing {offlineOrders.length} cached orders and {pendingOrders.length} pending orders. Some features may be limited.
//                             </p>
//                         </div>
//                     </div>
//                 </div>
...
//                                 key={s}
//                                 onClick={() => setStatus(s)}
//                                 className={`text-xs sm:text-sm font-semibold transition-all duration-200 ease-in-out px-3 sm:px-4 py-2 rounded-lg whitespace-nowrap ${status === s ? "bg-[#02ca3a] text-black shadow-lg" : "text-[#ababab] hover:bg-[#444444]"
//                                     }`}
//                                 aria-pressed={status === s}
//                             >
//                                 {s}
...
//                                         if (d !== "Custom") setSelectedDate("");
//                                     }}
//                                     className={`text-xs sm:text-sm font-medium px-3 sm:px-4 py-1 rounded-lg transition-colors whitespace-nowrap ${dateFilter === d ? "bg-[#444444] text-[#f5f5f5] shadow-inner" : "text-[#ababab] hover:bg-[#444444]"
//                                         }`}
//                                     aria-pressed={dateFilter === d}
//                                 >
//                                     {d === "All" ? "All Dates" : d}
...
//                                     key={type}
//                                     onClick={() => setOrderType(type)}
//                                     className={`text-xs sm:text-sm font-medium px-3 sm:px-4 py-1 rounded-lg transition-colors whitespace-nowrap ${orderType === type ? "bg-[#444444] text-[#f5f5f5] shadow-inner" : "text-[#ababab] hover:bg-[#444444]"
//                                         }`}
//                                     aria-pressed={orderType === type}
//                                 >
//                                     {type}
...
//             {/* ======  BULK-COMPLETE BUTTON  ====== */}
//             {bulkStats?.InProgress > 0 && isOnline && (
//                 <div className="px-4 sm:px-6 lg:px-8 py-3 bg-[#262626] border-b border-[#333333]">
//                     <button
...
//                     >
//                         <CheckCircle className="w-5 h-5" />
//                         Complete All {bulkStats.InProgress} Order{bulkStats.InProgress > 1 ? 's' : ''}
//                     </button>
//                 </div>
//             )}

//             {/* ======  BULK-COMPLETE CONFIRM MODAL  ====== */}
//             {showBulkCompleteModal && (
//                 <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-60 z-50 p-4">
...
//                             Complete {bulkStats.InProgress} order{bulkStats.InProgress > 1 ? 's' : ''}?
//                         </h3>
//                         <p className="text-sm text-gray-400 mb-4">
//                             This will mark every <span className="text-green-400 font-medium">In-Progress</span> order (matching current filters) as <span className="text-green-400 font-medium">Completed</span>.
//                         </p>
...
//                                             paymentMethod: paymentMethod === 'All' ? undefined : paymentMethod,
//                                         };
//                                         const { success, modifiedCount, message } = await completeMultipleOrders(payload);
//                                         if (success) {
...
//                             key={order._id}
//                             className="relative"
//                             ref={index === sortedFilteredOrders.length - 1 ? lastOrderRef : null}
//                         >
//                             <OrderCard order={order} />
//                             {order._isPending && (
//                                 <div className="absolute top-2 right-2 bg-orange-500 text-white text-xs font-bold px-2 py-1 rounded-full shadow-lg">
...
//                 ) : (
//                     <div className="col-span-full flex flex-col justify-center items-center h-full min-h-[300px] sm:min-h-[400px]">
//                         <svg className="w-12 h-12 sm:w-16 sm:h-16 text-gray-600 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
//                             value={adminPassword}
//                             onChange={(e) => setAdminPassword(e.target.value)}
//                             onKeyPress={(e) => e.key === 'Enter' && setShowDetailedBreakdown(true) && setShowPasswordModal(false)}
//                         />
//                         <div className="flex justify-end gap-2 sm:gap-3">
//                             <button
//                                 onClick={() => {
...
//                                     setAdminPassword("");
//                                 }}
//                                 className="px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg bg-gray-700 text-gray-200 hover:bg-gray-600 text-sm sm:text-base"
//                             >
//                                 Cancel
//                             </button>
//                             <button
...
//                                     setShowPasswordModal(false);
//                                 }}
//                                 className="px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-sm sm:text-base"
//                             >
//                                 Confirm
//                             </button>
//                         </div>
...
import { useOrderStats, usePaymentTotals } from "../hooks/orderData API optimization hooks/useOrderFilters";
import { useTodayAnalytics } from "../hooks/orderData API optimization hooks/useAnalytics";
import { completeAllOrders } from '../https/index';   //  <-- bulk-complete helper
...
const Orders = () => {
    ...
    const { stats: orderStats, isLoading: isLoadingStats, refetch: refetchStats } = useOrderStats(filters, isOnline);
    const { totals: paymentTotals, isLoading: isLoadingPaymentTotals, refetch: refetchPaymentTotals } = usePaymentTotals(filters, isOnline);
    const { todayData, isLoading: isLoadingToday } = useTodayAnalytics();

    /* --------------  EXTRA STATS FOR BULK BUTTON  -------------- */
    const { stats: bulkStats } = useOrderStats(
        {
            status: undefined,                       // ignore local status
            dateFilter,
            startDate: dateFilter === "Custom" && selectedDate ? selectedDate : undefined,
            endDate: dateFilter === "Custom" && selectedDate ? selectedDate : undefined,
            orderType: orderType === "All" ? undefined : orderType,
    ...
    console.log("BulkStats:", bulkStats);
    console.log("ByStatus:", bulkStats?.byStatus);
    ...
    useEffect(() => {
        const handleOnline = () => {
            setIsOnline(true);
            enqueueSnackbar("Connection restored! Syncing data...", { variant: "success" });
            refetchOrders(); refetchStats(); refetchPaymentTotals(); fetchAndCacheRecentOrders();
        };
        const handleOffline = () => {
            setIsOnline(false);
            enqueueSnackbar("You are offline. Showing cached orders.", { variant: "warning" });
    ...
    useEffect(() => {
        ...
        const handleOrderUpdate = (data) => {
            ...
            if (relevant.includes(data.action)) {
                refetchOrders(); refetchStats(); refetchPaymentTotals();
                const msg = {
                    new_order: 'New order received', items_ready: 'Order items marked ready',
                    status_changed: 'Order status updated', order_modified: 'Order modified', order_deleted: 'Order deleted'
                };
                enqueueSnackbar(msg[data.action] || 'Order updated', { variant: data.action === 'new_order' ? 'success' : 'info', autoHideDuration: 3000 });
            }
        };
        socket.on('orderUpdate', handleOrderUpdate);
        return () => socket.off('orderUpdate', handleOrderUpdate);
    ...
    /* --------------  DERIVED LISTS  -------------- */
    const ordersArray = useMemo(() => {
        if (isOnline) return infiniteOrders;
        const combined = [...offlineOrders];
        const pending = pendingOrders.map(o => ({ ...o, _isPending: true, _id: o._id || `pending-${Date.now()}-${Math.random()}` }));
    ...
    /* --------------  HANDLERS  -------------- */
    const handlePrintSalesReport = useCallback(async () => {
        if (!isOnline) { enqueueSnackbar("You must be online to print sales report!", { variant: "error" }); return; }
        setIsPrintingReport(true);
        try {
            await printSalesReport(todayData || calculateSalesReportManually());
            enqueueSnackbar("Sales report printed successfully!", { variant: "success" });
        } catch (error) {
            console.error("Error printing sales report:", error);
            enqueueSnackbar("Failed to print sales report!", { variant: "error" });
    ...
    const calculateSalesReportManually = useCallback(() => {
        const today = new Date().toDateString();
        const todayOrders = sortedFilteredOrders.filter(o => new Date(o.createdAt).toDateString() === today && o.orderStatus === "Completed");
        /* --------------  simplified summary logic  -------------- */
        let cash = 0, online = 0, benefit = 0, totalTax = 0, totalDiscount = 0, totalOrders = 0;
        todayOrders.forEach(o => {
            const amt = o.bills?.totalWithTax || 0;
            const tax = o.bills?.tax || 0;
            const disc = o.bills?.discountAmount || 0;
            totalTax += tax; totalDiscount += disc; totalOrders++;
            const pm = o.paymentMethod?.toLowerCase() || 'cash';
            if (pm === 'cash') cash += amt; else if (pm === 'online') online += amt; else if (pm === 'benefit') benefit += amt;
        });
        return {
            date: new Date().toLocaleDateString('en-GB', { year: 'numeric', month: 'long', day: 'numeric' }),
            totalOrders,
            totalSales: (cash + online + benefit).toFixed(3),
            paymentMethods: { cash: cash.toFixed(3), online: online.toFixed(3), benefit: benefit.toFixed(3) },
            taxSummary: { total: totalTax.toFixed(3) },
            discountSummary: { total: totalDiscount.toFixed(3) }
        };
    }, [sortedFilteredOrders]);
    ...
    if ((isLoadingOrders && isOnline) || isLoadingOffline) {
        return (
            <section className="bg-[#1f1f1f] h-[calc(100vh-5rem)] flex items-center justify-center">
                <div className="text-center">
                    <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#02ca3a] mx-auto mb-4"></div>
                    <p className="text-[#f5f5f5] text-lg">{isOnline ? 'Loading orders...' : 'Loading cached orders...'}</p>
                </div>
            </section>
        );
    }
    ...
    return (
        <section className="bg-[#1f1f1f] min-h-screen pb-20 md:pb-24">
                    ...
                    {(getUserRole() === "admin" || showDetailedBreakdown) && (
                        <div className="bg-[#02ca3a] text-black font-bold px-3 sm:px-6 py-2 sm:py-3 rounded-xl shadow-lg">
                            <div className="text-left">
                                ...
                                <p className="text-base sm:text-lg lg:text-xl font-extrabold">{paymentTotals?.Total || "0.000"}</p>
                            </div>
                        </div>
                    )}
                </div>
            ...
            {!isOnline && (
                <div className="bg-orange-500/20 border-l-4 border-orange-500 px-4 sm:px-6 lg:px-8 py-3">
                    <div className="flex items-center gap-3">
                        <WifiOff className="w-5 h-5 text-orange-400 flex-shrink-0" />
                        <div>
                            <p className="text-sm font-semibold text-orange-400">You are currently offline</p>
                            <p className="text-xs text-orange-300">Showing {offlineOrders.length} cached orders and {pendingOrders.length} pending orders. Some features may be limited.</p>
                        </div>
                    </div>
                </div>
            )}
            ...
            {/* -------  BULK-COMPLETE BUTTON  ------- */}
            {localBulkStats?.byStatus?.["In Progress"] > 0 && isOnline && (
            ...
            {/* {bulkStats?.byStatus?.["In Progress"] > 0 && isOnline && (
                <button
                    className="bulk-complete-btn"
                    onClick={() => setIsBulkModalOpen(true)}
                >
                    Complete All {bulkStats?.byStatus?.["In Progress"] ?? 0} Orders
                </button>
            )} */}
                ...
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-60 z-50 p-4">
                    <div className="bg-[#2e2c2c] p-6 rounded-xl shadow-2xl w-full max-w-sm border border-gray-700">
                        ...
                        <div className="flex justify-end gap-3">
                            ...
                            }} className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white">Confirm</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {showDetailedBreakdown && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-60 z-50 p-4">
                    <div className="bg-[#2e2c2c] p-4 sm:p-6 rounded-xl shadow-2xl w-full max-w-[320px] sm:max-w-[450px] border border-gray-700">
                        ...
                        <div className="space-y-3 mb-6">
                            <div className="flex items-center justify-between p-3 bg-[#1a1a1a] rounded-lg"><div className="flex items-center gap-3"><div className="w-3 h-3 rounded-full bg-green-500"></div><span className="text-sm sm:text-base font-medium text-[#f5f5f5]">Cash Payments</span></div><span className="text-base sm:text-lg font-bold text-[#02ca3a]">{paymentTotals?.Cash || "0.000"}</span></div>
                            <div className="flex items-center justify-between p-3 bg-[#1a1a1a] rounded-lg"><div className="flex items-center gap-3"><div className="w-3 h-3 rounded-full bg-blue-500"></div><span className="text-sm sm:text-base font-medium text-[#f5f5f5]">Online Payments</span></div><span className="text-base sm:text-lg font-bold text-[#02ca3a]">{paymentTotals?.Online || "0.000"}</span></div>
                            <div className="flex items-center justify-between p-3 bg-[#1a1a1a] rounded-lg"><div className="flex items-center gap-3"><div className="w-3 h-3 rounded-full bg-purple-500"></div><span className="text-sm sm:text-base font-medium text-[#f5f5f5]">Benefit Payments</span></div><span className="text-base sm:text-lg font-bold text-[#02ca3a]">{paymentTotals?.Benefit || "0.000"}</span></div>
                            <div className="flex items-center justify-between p-4 bg-[#02ca3a] rounded-lg mt-4"><span className="text-base sm:text-lg font-bold text-black">Total Amount</span><span className="text-lg sm:text-xl font-extrabold text-black">{paymentTotals?.Total || "0.000"}</span></div>
                            <button onClick={handlePrintSalesReport} disabled={isPrintingReport || !isOnline} className="w-full bg-purple-600 hover:bg-purple-700 text-white font-semibold px-4 py-2.5 rounded-xl shadow-lg transition-all duration-200 flex justify-center gap-2 text-sm disabled:opacity-50 disabled:cursor-not-allowed" title="Print Today's Sales Report"><Printer className="w-5 h-5" />{isPrintingReport ? 'Printing...' : 'Sales Report'}</button>
                        </div>
