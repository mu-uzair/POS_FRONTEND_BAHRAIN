...
//         return ordersArray
//             .map((order) => {
//                 // ✅ Filter kitchen items: exclude "Ready" (capital R) but include "pending"
//                 const kitchenItems = order.items?.filter(
//                     (item) =>
//                         item.section &&
//                         item.section.toLowerCase() === "kitchen" &&
//                         item.status !== "Ready" // ✅ Only exclude capital "Ready"
//                 );
...
//                 return {
//                     _id: order._id,
//                     orderNo: order.orderNo || "N/A", // ✅ Add orderNo
...
//             // ✅ Check if the order has any KITCHEN items before playing sound
//             const hasKitchenItems = data.order?.items?.some(
//                 (item) => item.section?.toLowerCase() === "kitchen"
//             );

//             // 🔔 Only play sound if it's an alert event AND has kitchen items
//             if (isAlertEvent && hasKitchenItems && isSoundEnabled && isAudioReady) {
//                 playAlertSound();
//                 console.log("🍳 Kitchen alert sound played - order contains kitchen items");
//             } else if (isAlertEvent && !hasKitchenItems) {
//                 console.log("🔇 No sound - order has no kitchen items");
//             }
...
//                 // Only show notification if order has kitchen items
//                 if (hasKitchenItems) {
//                     enqueueSnackbar(
...
//         };
//     }, [queryClient, isSoundEnabled, isAudioReady, playAlertSound]);
...
//                                 <div className="bg-[#F6B100] text-black font-bold px-3 py-1 rounded-lg text-base">
//                                     {order.orderNo}
//                                 </div>
...
//                                             </p>
//                                             {/* ✅ Show NEW badge for pending items */}
//                                             {item.status === "pending" && (
...
const KitchenSection = () => {
        ...
        const unlockAudio = async () => {
            if (!isAudioReady) {
                try {
                    ...
                    orderAlert.currentTime = 0;
                    setIsAudioReady(true);
                    console.log("✅ Kitchen audio context unlocked");
                } catch (e) {
                    console.warn("Audio unlock failed (expected on first load):", e.message);
                }
            }
    ...
    // ✅ 1. Wrap playAlertSound in useCallback
    const playAlertSound = useCallback(async () => {
        try {
            ...
            if (playPromise !== undefined) {
                await playPromise;
                console.log("🔔 Kitchen alert sound played successfully");
            }
        } catch (error) {
            console.warn("⚠️ Could not play alert sound:", error.message);
    ...
    // ✅ 2. FIXED socket handler - use data.data.items (same as Grill)
    useEffect(() => {
        const handleOrderUpdate = (data) => {
            console.log("🍳 Kitchen socket update:", data);

            const isAlertEvent = 
                data.action === 'new_order' || 
                data.action === 'order_modified';

            // ✅ 3. CRITICAL FIX: Use data.data.items (NOT data.order.items)
            const hasKitchenItems = data.data?.items?.some(
                (item) => item.section === "Kitchen" // ✅ Capital K to match backend
            );

            console.log("Has kitchen items:", hasKitchenItems);

            // 🔔 Play sound if: alert event + has kitchen items + sound enabled + audio ready
            if (isAlertEvent && hasKitchenItems && isSoundEnabled && isAudioReady) {
                playAlertSound();
            ...
            // Invalidate queries and show notifications
            if (data.action === 'new_order' ||
                data.action === 'items_ready' ||
                data.action === 'status_changed' ||
                data.action === 'order_modified' ||
                ...
                if (hasKitchenItems) {
                    enqueueSnackbar(
                        `Kitchen order updated: ${data.action.replace('_', ' ')}`, 
                        { variant: isAlertEvent ? 'warning' : 'info' }
                    );
                }
            }
