...
//         return ordersArray
//             .map((order) => {
//                 // ✅ Filter grill items: exclude "Ready" (capital R) but include "pending"
//                 const grillItems = order.items?.filter(
//                     (item) =>
//                         item.section &&
//                         item.section.toLowerCase() === "grill" &&
...
//             // ✅ Check if the order has any GRILL items before playing sound
//             const hasGrillItems = data.order?.items?.some(
//                 (item) => item.section?.toLowerCase() === "grill"
//             );

//             // 🔔 Only play sound if it's an alert event AND has grill items
//             if (isAlertEvent && hasGrillItems && isSoundEnabled && isAudioReady) {
//                 playAlertSound();
//                 console.log("🔥 Grill alert sound played - order contains grill items");
//             } else if (isAlertEvent && !hasGrillItems) {
//                 console.log("🔇 No sound - order has no grill items");
//             }
...
//                 // Only show notification if order has grill items
//                 if (hasGrillItems) {
//                     enqueueSnackbar(
//                         `Grill order updated: ${data.action.replace('_', ' ')}`, 
//                         { variant: isAlertEvent ? 'warning' : 'info' }
...
//                                     <div className="bg-[#0f172a] rounded-lg p-3 sm:p-4 my-4 flex-grow">
//                                         {order.items.map((item, index) => (
//                                             <div key={index} className="mb-2 sm:mb-3 p-1 border-b border-gray-700 last:border-b-0">
//                                                 <div className="flex items-center justify-between">
//                                                     <p className="text-white text-base sm:text-lg font-bold">
...
const GrillSection = () => {
    ...
    // ✅ Wrap playAlertSound in useCallback to prevent recreation
    const playAlertSound = useCallback(async () => {
        try {
            ...
            if (playPromise !== undefined) {
                await playPromise;
                console.log("🔔 Grill alert sound played successfully");
            }
        } catch (error) {
            console.warn("⚠️ Could not play alert sound:", error.message);
  ...
  useEffect(() => {
    const handleOrderUpdate = (data) => {
        ...
        // ✅ FIXED: Items are in data.data.items, not data.items
        const hasGrillItems = data.data?.items?.some(
            (item) => item.section === "Grill" // ✅ Capital "G" to match your data
        );
        console.log("Order has grill items:", hasGrillItems);
    ...
    }, [queryClient, isSoundEnabled, isAudioReady, playAlertSound]); // ✅ Added playAlertSound
